...
<head>
    ...
    <title>Deatly Pagamentos - Cartão</title>
...
<div class="header">
    <div class="container">
        ...
        <div class="header-grid-text">
            <div class="header-content">
                <h4><?php print_r(ucfirst($_SESSION['pmt_ss']['first_name'])); ?>, seu pedido foi aprovado.</h4>
                ...
                ?>.</span><span> Código de pagamento: </span><span> <?php print_r($_SESSION['payment']->id); ?>.</span>
                <input type="button" value="Salvar informações" class="copy-payment-infos">
            </div>
        </div>
    </div>
</div>
<div class="status">
    <div class="container">        
        <div class="status-group">
            ...
            <div class="status-grid" style="border-bottom: solid 10px #545358;">
                <div class="status-content">
                    <div class="card-ready-circle"></div>
                </div>
            </div>
        </div>
        <div class="status-group">
    ...
    <div class="container">
        <div class="informations-grid">
            <div class="informations-content">
                <h2>Por gentileza, leia as instruções abaixo.</h2>
                <p>Em instantes, você receberá um email informando a confirmação da sua compra e os detalhes do seu pedido.</p>
                <p>Agora que seu pedido foi confirmado, basta aguarda um pouquinho. Seu pedido será enviado em até 5 dias úteis.</p>
                <p>Não se preocupe, assim que seu produto for enviado você receberá um email informando o código de rastreio para acompanhar a trajetória do seu pedido.</p>
                <p>Sinta-se à vontade para tirar suas dúvidas através de nosso email contato@deatly.com e/ou WhatsApp +5506581758763.</p>
            </div>
        </div>      
    </div>
</div>
...
Dados da transação
    ...
    Transação n. <?php print_r($_SESSION['payment']->id); ?>

    <?php echo strtoupper($pmt_dc) ?>

    <?php if ($pmt_v1n != '') {echo $pmt_v1.'x '.ucfirst($pmt_v1n).' | ';}else{echo '';} ?><?php if ($pmt_v2n != '') {echo $pmt_v2.'x '.ucfirst($pmt_v2n).' | ';}else{echo '';} ?><?php if ($pmt_v3n != '') {echo $pmt_v3.'x '.ucfirst($pmt_v3n).' | ';}else{echo '';} ?><?php if ($pmt_v4n != '') {echo $pmt_v4.'x '.ucfirst($pmt_v4n).' | ';}else{echo '';} ?><?php if ($pmt_v5n != '') {echo $pmt_v5.'x '.ucfirst($pmt_v5n).' | ';}else{echo '';} ?>
    
    R$ <?php echo $pmt_ta ?>: Pago.

<?php print_r(ucfirst($_SESSION['pmt_ss']['first_name'])); ?>, obrigado pela sua compra. Você receberá um email contendo as mesmas informações mencionadas acima.
</textarea>
...
<div class="print">
    <div class="container">
        ...
        /*    echo '<pre>',print_r($_SESSION['payment']),'</pre>';
            echo '<pre>',print_r($_SESSION['pmt_ss']),'</pre>'; */
